...
def load_bgg_csv(
    ...
    Returns a dict mapping BGG ID to game name.
    """
    games: dict[int, str] = {}
    wishlist_count = 0
...
def find_matching_games(
    ...
    bgg_games: dict[int, str],
) -> list[Game]:
    """Find games that appear in both the library and the user's BGG list."""
    matching = []

    for bgg_id in bgg_games:
        if bgg_id in library_games:
            matching.append(library_games[bgg_id])

    return sorted(matching, key=lambda g: g.name.lower())
...
def main():
    ...
    for game in available:
        ...
        output_lines.append(f"- {game.name}{catalog}{status}")
